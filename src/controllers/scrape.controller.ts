 ...
 */
function startBackgroundPolling(requestId: string, runId: string) {
  let attempts = 0;
  ...
  const interval = setInterval(async () => {
    ...
    try {
      ...
      const currentReq = await prisma.scrapeRequest.findUnique({
        where: { id: requestId }
      });
      ...
      const run = await ApifyService.getRunDetails(runId);
      if (!run) {
        clearInterval(interval);
        return;
      }
      ...
      if (apifyStatus === 'SUCCEEDED') {
        ...
        await prisma.scrapeRequest.update({
          ...
          data: {
            ...
            responses: {
              create: {
                data: items as any // Prisma Json supports arrays
              }
            },
            logs: {
              create: {
...
export class ScrapeController {
  ...
  static async triggerScrape(req: Request, res: Response) {
    ...
    try {
      ...
      scrapeRequest = await prisma.scrapeRequest.create({
        data: {
          actorName,
          status: 'PENDING',
          inputData: inputData,
      ...
      const run = await ApifyService.triggerActor(actorName, inputData, scrapeRequest.id);

      // 3. Update the request with Apify run details and update status to RUNNING
      scrapeRequest = await prisma.scrapeRequest.update({
   ...
   */
  static async triggerInstagramScrape(req: Request, res: Response) {
    ...
    const inputData = {
      directUrls: usernames.map((u: string) => u.startsWith('http') ? u : `https://www.instagram.com/${u}/`),
   ...
   */
  static async getScrapeStatus(req: Request, res: Response) {
   ...
   */
  static async getScrapeResults(req: Request, res: Response) {
    const id = req.params.id as string;

    try {
      const response = await prisma.scrapeResponse.findFirst({
        where: { scrapeRequestId: id }
      });

      if (!response) {
        // Check if request exists
        const request = await prisma.scrapeRequest.findUnique({ where: { id } });
        if (!request) {
          return res.status(404).json({ error: 'Scrape request not found.' });
        }
